...
import { type IItemProps, type IProps, type TEvent } from './types';
import {
  binarySearch,
  checkIfInteractive,
  getTranslateOffset,
  ...
  onMouseOrTouchStart = (e: MouseEvent & TouchEvent) => {
    ...
    if (
      ...
      (this.props.values[index] &&
        (
          this.props.values[index] as unknown as {
            disabled: boolean;
          }
        ).disabled)
    ) {
    ...
    if (
      ...
      this.setState({
        scrollingSpeed: Math.min(
          Math.round(
            (AUTOSCROLL_ACTIVE_OFFSET - (viewportHeight - clientY)) / AUTOSCROLL_SPEED_RATIO
          ),
          Math.round((delta - AUTOSCROLL_DELTA_THRESHOLD) / AUTOSCROLL_SPEED_RATIO)
        ),
        scrollWindow: true,
      });
      ...
      this.setState({
        scrollingSpeed: Math.max(
          Math.round((AUTOSCROLL_ACTIVE_OFFSET - clientY) / -AUTOSCROLL_SPEED_RATIO),
          Math.round((delta + AUTOSCROLL_DELTA_THRESHOLD) / AUTOSCROLL_SPEED_RATIO)
        ),
        scrollWindow: true,
      });
      ...
      if (height + 20 < this.listRef.current!.scrollHeight) {
        ...
        if (clientY - top < AUTOSCROLL_ACTIVE_OFFSET && delta < -AUTOSCROLL_DELTA_THRESHOLD) {
          scrollingSpeed = Math.max(
            Math.round((AUTOSCROLL_ACTIVE_OFFSET - (clientY - top)) / -AUTOSCROLL_SPEED_RATIO),
            Math.round((delta + AUTOSCROLL_DELTA_THRESHOLD) / AUTOSCROLL_SPEED_RATIO)
          );
          // (down)
        } else if (
          ...
          scrollingSpeed = Math.min(
            Math.round((AUTOSCROLL_ACTIVE_OFFSET - (bottom - clientY)) / AUTOSCROLL_SPEED_RATIO),
            Math.round((delta - AUTOSCROLL_DELTA_THRESHOLD) / AUTOSCROLL_SPEED_RATIO)
          );
        }
